...
export default {
  ...
  mutations: {
    ...
    setDisplay(state: displayState, payload: any) {
      state.bigPhoto = payload[0].size[0].color[0].photo[0];
    ...
    //更新大照片
    upDataBigPhoto(state: displayState, bigPhoto: String) {
      state.bigPhoto = bigPhoto;
    },
    // //选择size
      ...
      state.allReviewDisPlay.filter((index: any) => {
        if (index.sizeName === payload) {
          ...
          if (isDisplay.length === 0) {
            ...
            state.bigPhoto = state.display[0].photo[0];
          } else {
  ...
  getters: {
    getBigPhoto: (state: displayState) => {
      console.log("in getDBigPhoto method", state.bigPhoto);
